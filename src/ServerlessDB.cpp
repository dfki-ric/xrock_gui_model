...
namespace xrock_gui_model {
  ...
  std::vector<std::pair<std::string, std::string>> ServerlessDB::requestModelListByDomain(const std::string& domain) {
    return serverless->requestModelListByDomain("ComponentModel", mars::utils::toupper(domain));
  }
  ...
  std::vector<std::string> ServerlessDB::requestVersions(const std::string& domain, const std::string &model) {
    ...
    return serverless->requestVersions("ComponentModel", mars::utils::toupper(domain), model);
  }
  ...
  ConfigMap ServerlessDB::requestModel(const std::string &domain,
    ...
    std::vector<XTypePtr> xtypes = serverless->findByJsonPropString("ComponentModel", props.toJsonString(), limit ? 3 : -1);
    if (xtypes.size() == 0) {
